...
def send_friend_request(sender, receiver_id):
    ...
    existing_req = FriendRequest.objects.filter(sender=sender, receiver=receiver, status='pending').first()
    if existing_req:
        raise ValidationError("Friend request already sent")

    # Reverse pending request exists?
    reverse_req = FriendRequest.objects.filter(sender=receiver, receiver=sender, status='pending').first()
    if reverse_req:
...
def block_user(blocker, blocked_user_id):
    ...
    Friendship.objects.filter(Q(user1=blocker, user2=blocked_user) | Q(user1=blocked_user, user2=blocker)).delete()
...
def unblock_user(blocker, blocked_user_id):
    deleted, _ = UserBlock.objects.filter(blocker=blocker, blocked_user_id=blocked_user_id).delete()
    if deleted == 0:
         raise NotFound("Block record not found")
    return True


def get_nearby_users(current_user, user_type=None, radius=50.0, search_query='', include_friends=True, city=None, state=None):
    user_point = None
    ...
    blocked_ids = list(UserBlock.objects.filter(blocker=current_user).values_list('blocked_user_id', flat=True))
    blocked_by_ids = list(UserBlock.objects.filter(blocked_user=current_user).values_list('blocker_id', flat=True))
    
    # Exclude existing friends
    friendships = Friendship.objects.filter(Q(user1=current_user) | Q(user2=current_user))
    friend_ids = [f.user2_id if f.user1_id == current_user.id else f.user1_id for f in friendships]
